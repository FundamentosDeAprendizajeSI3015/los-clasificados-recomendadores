"""
Script de entrenamiento y evaluación utilizando Árbol de Decisión (DecisionTreeClassifier).

Este módulo provee la funcionalidad para entrenar y evaluar modelos basados en 
árboles de decisión para los cuatro objetivos (targets) del proyecto, integrando 
el preprocesamiento de datos (imputación y codificación) de forma automática.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Script general de Árbol de Decisión: entrenamiento y evaluación para los 4 objetivos"
    )
    parser.add_argument(
        "--data",
...
def build_pipeline(random_state: int) -> Pipeline:
    """
    Construye el pipeline de preprocesamiento y clasificación con un Árbol de Decisión.
...
def train_target(df: pd.DataFrame, target: str, args: argparse.Namespace) -> None:
    """
    Entrena un modelo de Árbol de Decisión para un objetivo específico y guarda modelo y métricas.
...
def evaluate_target(df: pd.DataFrame, target: str) -> None:
    """
    Carga el modelo de Árbol de Decisión entrenado y genera predicciones/evaluación.
